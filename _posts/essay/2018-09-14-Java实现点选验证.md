...
layout: post
title: "Java实现点选验证码"
description: Java实现点选验证码
category: 个人随笔
---

# 效果

 话不多说，先看实现的效果,其中方框为点击区域，真实场景要去掉此方框
  - 前端
  
 ![image](https://jasperxgwang.github.io/images/essay/choose_view.jpg)
 
 - 后台
 
 ![image](https://jasperxgwang.github.io/images/essay/choose_list.jpg)
...
- 在背景图片中随机获取中文字符位置
- 设置中文旋转随机角度、字体颜色
- 保存生成文字的坐标集合、提示信息、验证码图片
- 前端根据提示信息获取相应的点击坐标集合，提交到服务端进行基础验证

![image](https://jasperxgwang.github.io/images/essay/验证流程.jpg)
...
```java
    ...
    //汉字颜色随机范围
    private static Color[] colors = {Color.YELLOW, Color.ORANGE, Color.PINK};
     ...
     * 生成文字点选验证码
     */
    public static ResultPack<ImgV2InfoVo> createTextSelectionImg(String sourceImgUrl) {
        try {
            //生成背景图片
            ...
            for (int i = 0; i < arr.length; i++) {
                ...
                if (place == 1) {
                    x = RandomUtil.randomInt(width / 2) + 10;
                    y = RandomUtil.randomInt(height / 2) + 10;
                }
                if (place == 2) {
                    x = RandomUtil.randomInt(width / 2) + 10;
                    y = RandomUtil.randomInt(height / 2, height - 30);
                }
                if (place == 3) {
                    x = RandomUtil.randomInt(width / 2, width - 30);
                    y = RandomUtil.randomInt(height / 2) + 10;
                }
                if (place == 4) {
                    x = RandomUtil.randomInt(width / 2, width - 30);
                ...
                if (place == 5) {
                    x = RandomUtil.randomInt(width / 2 - 10, width / 2 + 10);
                    y = RandomUtil.randomInt(height / 2 - 10, height / 2 + 10);
                }
                ...
                affineTransform.rotate(Math.toRadians(RandomUtil.randomInt(50, 85)), 0, 0);
            ...
            //存储图片服务器
            String captchaImg = ImgServerUtils.uploadImage2URL(combined);

            ImgV2InfoVo vo = new ImgV2InfoVo(captchaImg, tips, codeList);
            return ResultPack.succeed(vo);
        } catch (Exception e) {
...
- Linux线上环境部署时，发现生成文字有乱码
 
  原因是由于验证码图片生成过程中使用到了`宋体（simsun.ttf）`和`微软雅黑（微软vista雅黑.ttf）`,所以要在linux环境中安装相应的字体库
  
  链接：https://pan.baidu.com/s/1sqcBO0vdgKoGh9iEXNDE9Q 密码：7wwu
  
  解决方法
 
  - 拷贝中文字体到JRE目录 $JAVA_HOME/jre/lib/fonts,如图
   ![image](https://jasperxgwang.github.io/images/essay/jvm_fonts.jpg)
  - 重启服务
