...
![image](https://jasperbalcony.github.io/images/concurrent/1.png)

接着我们进行一些操作：

- 1、线程put1执行 put(1)操作，由于当前没有配对的消费线程，所以put1线程入队列，自旋一小会后睡眠等待，这时队列状态如下：

![image](https://jasperbalcony.github.io/images/concurrent/2.png)

- 2、接着，线程put2执行了put(2)操作，跟前面一样，put2线程入队列，自旋一小会后睡眠等待，这时队列状态如下：

![image](https://jasperbalcony.github.io/images/concurrent/3.png)
...
![image](https://jasperbalcony.github.io/images/concurrent/4.png)

- 4、最后，再来一个线程take2，执行take操作，这时候只有put2线程在等候，而且两个线程匹配上了，线程put2被唤醒，
take2线程take操作返回了2(线程put2的数据)，这时候队列又回到了起点，如下所示：

![image](https://jasperbalcony.github.io/images/concurrent/5.png)
